...
const StarRating: React.FC<StarRatingProps> = ({ rating, isLabelRequired=true , isEditable, onClick, numberOfStars=5}) => {
    const lowerBound = Math.floor(rating);
    const isHalfStar = lowerBound < rating;
    const hollowStars = numberOfStars - Math.ceil(rating);

    return (
        <div className='flex gap-3 rounded-lg items-center flex-col md:flex-row'>
